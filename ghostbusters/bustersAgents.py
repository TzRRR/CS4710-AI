...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghost in livingGhostPositionDistributions:
            ghost_positions.append(ghost.argMax())

        def dist_from_pacman(ghostPosition):
            return self.distancer.getDistance(pacmanPosition, ghostPosition)

        closest_ghost_position = min(ghost_positions, key=dist_from_pacman)

        def successor_distance(action):
            newPacmanPosition = Actions.getSuccessor(pacmanPosition, action)
            return self.distancer.getDistance(newPacmanPosition, closest_ghost_position)

        greedyAction = min(legal, key=successor_distance)
        return greedyAction
